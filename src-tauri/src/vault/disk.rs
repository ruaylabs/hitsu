...
//! save with `HitsuError::ExternalModification` instead of silently
...
pub fn ensure_unmodified(path: &Path, expected: &[u8; 32]) -> HitsuResult<()> {
    let actual = sha256_file(path).map_err(|_| HitsuError::ExternalModification)?;
    if actual != *expected {
        return Err(HitsuError::ExternalModification);
    }
    Ok(())
}
...
mod tests {
    ...
    fn temp_file(data: &[u8]) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("hitsu-disk-test-{}", uuid::Uuid::new_v4()));
    ...
    fn test_modified_file_is_detected() {
        ...
        assert!(matches!(
            ...
            Err(HitsuError::ExternalModification)
        ));
        let _ = std::fs::remove_dir_all(path.parent().unwrap());
    }
